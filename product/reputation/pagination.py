"""Shared pagination bounds for every `product/reputation/*.py` list
endpoint -- identical constants/logic to `product/websites/pagination.py`/
`product/appointments/pagination.py`, but NOT imported from either: the
same "duplicate a tiny, genuinely shared utility rather than ride a
