"""Shared pagination bounds for every `product/websites/*.py` list
endpoint -- identical constants/logic to `product/appointments/pagination.py`/
`product/telephony/pagination.py`, but NOT imported from either: the same
"duplicate a tiny, genuinely shared utility rather than ride a
