...
/roles.py`) to grant this module's own `templates.snapshot` permissions
to the newly provisioned role -- the "module needing another module's
...
`product.templates` never imports `product.agency` directly, the same
rule every other module's own `event_handlers.py` already follows.

`owner`: full lifecycle (`create`, `read`, `apply`). `member`:
`create`/`read`, never `apply` -- applying a snapshot creates real
configuration in a (possibly different) tenant, the same
higher-risk-than-ordinary-CRUD tier `product/reputation/event_handlers.py`'s
own module docstring reserves for `cancel`/`deactivate`-shaped actions.
"""
...
from product.templates.permissions import SNAPSHOT_RESOURCE, grant_to_role

_OWNER_ROLE_NAME = "owner"
_MEMBER_ROLE_NAME = "member"

_OWNER_GRANTS: tuple[tuple[str, tuple[str, ...]], ...] = (
    (SNAPSHOT_RESOURCE, ("create", "read", "apply")),
)
_MEMBER_GRANTS: tuple[tuple[str, tuple[str, ...]], ...] = ((SNAPSHOT_RESOURCE, ("create", "read")),)
