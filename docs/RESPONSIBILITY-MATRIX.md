...
| Custom-domain → tenant resolution | B (interim C) | Generic multi-tenant capability, but SaaS-OS's ingress only resolves tenant by subdomain/header today. Build in this product now as a thin middleware wrapping `core.identity.get_tenant_context()`; propose upstream once a second product needs it |
