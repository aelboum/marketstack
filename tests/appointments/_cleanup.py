...
first, in dependency order (appointments/availability_rules before
calendars), mirroring tests/marketing/_cleanup.py's exact scoped/
...
_APPOINTMENTS_TABLES_LEAF_TO_ROOT = ("appointments", "availability_rules", "calendars")
...
def cleanup_tenant_tree(*tenant_ids_leaf_to_root: uuid.UUID) -> None:
    ...
    `appointments` before `availability_rules` before `calendars`), then
